#!/bin/bash
#nodes="83.212.121.127 83.212.115.199 83.212.116.154 83.212.125.101 83.212.117.118 83.212.117.119 83.212.117.127"
nodes="`getNodes.sh ycsb`"
echo -n "Client Nodes: "
echo $nodes
function run_workload {
	...
	do
		echo "Starting: $node"
		ssh $node '/opt/ycsb-0.1.4/bin/ycsb run cassandra-10 -P /opt/ycsb-0.1.4/workloads/workloada  -threads 20 -p maxexecutiontime=1000000000 -p hostsFile=/opt/hosts -p operationcount=1000000000 -p recordcount=50000 -p target=650 -p sinusoidal=true -p period=7200 -p offset=600 -P /opt/ycsb-0.1.4/XProbe.properties -s > /opt/ycsb-0.1.4/logs 2> /opt/ycsb-0.1.4/logs &'
	done
